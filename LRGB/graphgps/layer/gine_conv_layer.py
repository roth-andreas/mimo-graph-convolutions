...
class GINEConvESLapPE(pyg_nn.conv.MessagePassing):
    ...
    """
    def __init__(self, nn, piep, eps=0., train_eps=False, edge_dim=None, **kwargs):
        kwargs.setdefault('aggr', 'add')
        super().__init__(**kwargs)
        print('Using GINEConvESLapPE') 
        self.nn = nn
...
class GINEConvLayer(nn.Module):
    ...
    def __init__(self, dim_in, dim_out, dropout, residual):
        ...
        self.model = pyg_nn.GINEConv(gin_nn, eps=0.0 if cfg.gnn.self_loops else 1.0)
...
class GINEConvGraphGymLayer(nn.Module):
    ...
    def __init__(self, layer_config: LayerConfig, **kwargs):
        super().__init__()
        print('Using GINEConvGraphGymLayer') 
        gin_nn = nn.Sequential(
